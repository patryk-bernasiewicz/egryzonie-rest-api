...
router.get('/', async (req, res, next) => {
  ...
  await mailer
    .send(email, 'remind-password/remind', data)
    .catch(next);
...
router.get('/validate', async (req, res, next) => {
  ...
  if (!token) {
    return res
      .status(400)
      .json({ message: 'token must be specified' });
  }
...
router.post('/change', async (req, res, next) => {
  ...
  const remind = await PasswordRemind
    .findOne({ token })
    .populate('user')
    .catch(next);
  if (!remind) {
  ...
  const result = await User
    .findOneAndUpdate({ _id: remind.user._id }, { password })
    .catch(next);
